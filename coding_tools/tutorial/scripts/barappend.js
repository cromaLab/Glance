 	...
 	function toggleStatus(clicked_id) {
 		...
 		if(clicked_id == "setStart"){
			...
			$('#toggleButton.button').css('border', '1px solid #c6430a');
 			...
 			if(currentTimeRight - parseFloat(val_bar[0]) < 1){ //if end pin is trying to go less then a second after start pin
 				$( "#slider-range" + (parseFloat(numBars) - 1) ).slider( "values", 1, (parseFloat(currentTimeRight) + 1) );
 			}
 			else {
				...
				// if(true)//((num_check + 1) == (tester.length/3))
						...
						$( "#slider-range" + n ).slider({
							...
							slide: function( event, ui ) {
								...
								// 	return false;
								// }
								if((ui.values[1] - ui.values[0]) <= 0.7)
								{
									return false;
								}
